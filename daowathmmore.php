...
if (!isset($_SESSION['user_login'])) {
 ...
 if ($daowathmlastid >= 1) {
 			//getting username
		 $result = mysqli_query($con,"SELECT * FROM posts WHERE id ='$daowathmlastid'") or die(mysqli_error());
		 $name = mysqli_fetch_assoc($result);
		 $username = $name['added_by'];
		//timeline query table
		$getposts = mysqli_query($con,"SELECT * FROM daowat WHERE added_by ='$username' AND daowat_give !='0' AND id < $daowathmlastid ORDER BY id DESC LIMIT 7") or die(mysqli_error());
