	...
	while ($row = mysqli_fetch_assoc($password_query)) {
		...
		if ($oldpassword_md5 == $db_password) {
			if ($newpassword == $repear_password) {
				...
				$password_update_query = mysqli_query($con,"UPDATE users SET password='$newpassword_md5', confirmCode='$confirmCode' WHERE username='$user'");
				$error = "<p class='succes_echo'>Success! Your password updated.</p>";
				// send email
				$msg = "Welcome !  ".$db_first_name."
				...
				Successfully your Buddies password has been changed. 
						...
						";
						mail($db_email ,"Buddies Password Changed",$msg, "From:Buddies <no-reply@daowat.com>");
				}
			}else {
		...
		<li style="float: left;">
			<div class="settingsleftcontent">
				<ul>
					...
					<li><a href="password_update.php" style=" background-color: darkblue; border-radius: 3px; color: #fff;">Password</a></li>
