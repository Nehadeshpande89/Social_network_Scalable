...
if ($sendemail) {
	...
	$pub_email = mysqli_real_escape_string($con,$pub_email);
		...
		$info_submit_query = mysqli_query($con,"UPDATE users SET mobile='$mobile' WHERE username='$user'");
		$info_submit_query = mysqli_query($con,"UPDATE users SET pub_email='$pub_email' WHERE username='$user'");
...
if ($emaildata) {
	...
	$check_for_email = mysqli_query($con,"SELECT * FROM users WHERE email='$email'");
	...
	if ($email == "") {
		...
		//getting email
		$check_for_email1 = mysqli_query($con,"SELECT * FROM users WHERE username='$user'");
		...
		$info_submit_query = mysqli_query($con,"UPDATE users SET email='$email', confirmCode='$confirmCode' WHERE username='$user'");
		echo "<script>window.open('cbinfo_update.php','_self')</script>";
		$error = "<p class='succes_echo'>Email successfully changed.</p>";
						// send email
						$msg = "Successfully your Buddies email has been changed. 
						
						";
						mail($email ,"Buddies Email Changed",$msg, "From:Buddies <no-reply@daowat.com>");
...
<?php echo $error; ?>
	<ul>
		<li style="float: left;">
			<div class="settingsleftcontent">
				<ul>
					...
					<li><a href="cbinfo_update.php" style="background-color: darkblue; border-radius: 3px; color: #fff;">Contact and Basic Info</a></li>
		...
		<li style="float: right;">
			...
			<div class="uiaccountstyle">
				<form action="cbinfo_update.php" method="POST">
				<h2><p>Change Buddies Email</p></h2></br>
					Buddies Email: </br><input type="email" name="email" id="email" class="placeholder" size="43" value="<?php echo $db_email; ?>"> </br></br>
