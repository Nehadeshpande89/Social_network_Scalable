...
//check for noti
$check_for_post_noti = mysqli_query($con,"SELECT * FROM post_comments WHERE posted_to='$user' posted_by != '$user' AND opened='no' ORDER BY id DESC");
if($check_for_post_noti == true){
	$post_noti_num = mysqli_num_rows($check_for_post_noti);
}else $post_noti_num = 0;

$check_for_daowat_noti = mysqli_query($con,"SELECT * FROM daowat_comments WHERE daowat_to='$user' AND opened='no' ORDER BY id DESC");
$daowat_noti_num = mysqli_num_rows($check_for_daowat_noti);
...
if (isset($_POST['gotonoti'])) {
	...
	$dwtopened_query = mysqli_query($con,"UPDATE daowat_comments SET opened='yes' WHERE daowat_to='$user'");
	
}

//followers
if (isset($_POST['gotofollow'])) {
	$pstopened_query = mysqli_query($con,"UPDATE follow SET opened='yes' WHERE user_to='$user'");
...
<div class="header">
	<div class="headerMainmenu">
		<div>
			...
			<?php
			if ($user != '') {
				echo '<li class="logo"><a href="index.php" title="Go to Buddies Home" >Buddies</a></li>
...
"><a href="logout.php" title="Log me out" style="font-weight: bold; margin: 3px 8px; font-size: 14px; color: darkblue;">Logout</a>
</button></form></li>';
