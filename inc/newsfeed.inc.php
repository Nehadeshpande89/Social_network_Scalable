...
$get_fname_info = mysqli_query($con,"SELECT * FROM users WHERE username='$user_posted_to'");
...
$get_sharepost = mysqli_query($con,"SELECT * FROM posts WHERE id='$sharepostid'");
...
$sp_get_user_info = mysqli_query($con,"SELECT * FROM users WHERE username='$sp_added_by'");
...
$sp_add_by = $sp_get_info['first_name'];
		...
		//check for propic delete
					$sp_pro_changed = mysqli_query($con,"SELECT * FROM posts WHERE added_by='$sp_added_by' AND (discription='changed his profile picture.' OR discription='changed her profile picture.') ORDER BY id DESC LIMIT 1");
...
if (strlen($body) > 4000) {
		$body = substr($body, 0, 2000)." <a href='viewPost.php?pid=".$id."' style='text-decoration: none; color: darkblue;'>... read more</a>";
	}else {
		$body = $body;
	}
...
if($sharepostid != 0) {
	...
	if($sp_added_by == $added_by) {
		if ($gender_user_db == '1') {
			$discription = "shared his <a href='viewPost.php?pid=".$sharepostid."' style='text-decoration: none; padding-left: 0px; color: darkblue;'>".$post_item."</a>";
		}else if ($gender_user_db == '2') {
			$discription = "shared her <a href='viewPost.php?pid=".$sharepostid."' style='text-decoration: none; color: darkblue;'>".$post_item."</a>";
		}
	}else {
		$discription = "shared ".$sp_add_by."'s <a href='viewPost.php?pid=".$sharepostid."' style='text-decoration: none; padding-left: 0px; color: darkblue;'>".$post_item."</a>";
	}
}

//Get Relevant Comments
$get_comments = mysqli_query($con,"SELECT * FROM post_comments WHERE post_id='$id' ORDER BY id DESC LIMIT 3");
...
//getting all like
$get_like = mysqli_query($con,"SELECT * FROM post_likes WHERE post_id='$id' ORDER BY id DESC");
		...
		echo "<div style='min-height: 55px;' ><div style='float: left;'><img src='$profile_pic' style= 'border-radius: 22px'; title=\"$added_by\" height='45' width='45'  /></div>";
			...
			echo'<div class="posted_by">
				<span style="color: #9e9e9e;" >
					...
					echo'<a href="profile.php?u='.$added_by.'" style="text-decoration: none; color: darkblue;">'.$add_by.'</a><span> '.$discription.'</span>';
					}else {
					echo'<a href="profile.php?u='.$sp_added_by.'" style="text-decoration: none; color: darkblue;">'.$add_by.'</a> > <a href="profile.php?u='.$user_posted_to.'" style="text-decoration: none; color: darkblue;">'.$post_to_fname.'</a>';
					}
			...
			if($sharepostid != 0) {
				...
				if (strlen($sp_body) > 1500) {
						$sp_body = substr($sp_body, 0, 1500)." <a href='viewPost.php?pid=".$sp_id."' style='text-decoration: none; color: darkblue;'>... read more</a>";
					}else {
						$sp_body = $sp_body;
					}
					...
					<div style="padding: 10px;" >';
						...
						<div style=" padding-left: 10px; overflow: hidden; ">
							<span style="color: #9197a3;" >
								<span style="font-weight: bold;">
									<a href="profile.php?u='.$sp_added_by.'" style="text-decoration: none; color: darkblue;">'.$sp_add_by.'</a>
								</span>
							</span>
			...
			$like_query = mysqli_query($con,"SELECT * FROM post_likes WHERE user_name='$user' AND post_id='$id' ORDER BY id DESC LIMIT 1");
			$rows_uname = mysqli_num_rows($like_query);
			if ($rows_uname == 1) {
				echo "<a href='like.php?upid=".$id."' style='color: darkblue;' >Liked . $count_like</a>";
			}else {
