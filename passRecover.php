...
$check_pic = mysqli_query($con,"SELECT profile_pic, first_name, username FROM users WHERE username='$username' || email='$username'");
$get_pic_row = mysqli_fetch_assoc($check_pic);
$profile_pic_db = $get_pic_row['profile_pic'];
$pic_uname = $get_pic_row['username'];
$fullname_db = $get_pic_row['first_name'];
//check for propic delete
$pro_changed = mysqli_query($con,"SELECT * FROM posts WHERE added_by='$pic_uname' AND (discription='changed his profile picture.' OR discription='changed her profile picture.') ORDER BY id DESC LIMIT 1");
...
if (isset($_POST['searchId'])) {
	...
	$username_query = mysqli_query($con,"SELECT * FROM users WHERE username='$username' || email='$username'");
		...
		if ($get_block_fetch_query == 1 ) {
			...
			$succes_echoEmail = "
				...
				<form action='passRecover.php' method='POST'>
					<span style='font-size:  margin: 13px 0 0 13px;  font-weight: 800; color: darkblue'>".$fullname_db."</span></br>
...
$recovEmail = mysqli_real_escape_string($con,$recovEmail);
if (isset($_POST['searchRecovEmail'])) {
	$username = $_SESSION['username'];
	$searchRecovEmail_query = mysqli_query($con,"SELECT * FROM users WHERE (email='$recovEmail' AND username='$username')");
	...
	if ($searchRecovEmail_query_num >= 1) {
		...
		if (mysqli_query($con,$confirmCodeQuery)) {
			...
			";
			//mail($recovEmail,"Buddies Confirmation Code",$msg, "From:Buddies <no-reply@daowat.com>");
			$success_email_cnfrm = "
				<form action='passRecover.php' method='POST'>
...
if (isset($_POST['searchRecovEmail2'])) {
	...
	$searchRecovEmail_query = mysqli_query($con,"SELECT * FROM users WHERE (email='$user_con_email' AND username='$user_con_name')");
	$searchRecovEmail_query_num = mysqli_num_rows($searchRecovEmail_query);
	$usernm_fetch_query = mysqli_fetch_assoc($searchRecovEmail_query);
	$get_first_nm_fetch_query = $usernm_fetch_query['first_name'];
	if ($searchRecovEmail_query_num >= 1) {
		...
		if (mysqli_query($con,$confirmCodeQuery)) {
			...
			N.B. If it's not by you, delete this mail or forget it.
			";
			mail($user_con_email,"Buddies Confirmation Code",$msg, "From:Buddies <no-reply@daowat.com>");
			$success_email_cnfrm = "
				<form action='passRecover.php' method='POST'>
						Verification Code:</br></br><input type='text' name='confirmMailCode' class='placeholder' size='30' required autofocus></br></br>
					</hr>
...
$confirmMailCode = mysqli_real_escape_string($con,$confirmMailCode);
$newpassword = mysqli_real_escape_string($con,$newpassword);
$repear_password = mysqli_real_escape_string($con,$repear_password);
if (isset($_POST['confrmRessetpass'])) {
	$unameCnfrm = $_SESSION['final_uname'];
	if ($unameCnfrm != '') {
		$unameCnfrm_query = mysqli_query($con,"SELECT * FROM users WHERE username='$unameCnfrm'");
			...
			if ($confirmMailCode == $get_confirmCode_fetch_query) {
				if ($newpassword == $repear_password) {
					...
					if (mysqli_query($con,$updatePassQuery)) {
						...
						// send email
						$msg = "Successfully your Buddies password has been changed. 
						...
						";
						//mail($get_uEmail_fetch_query ,"Buddies Password Changed",$msg, "From:Buddies <no-reply@daowat.com>");
					}
				}else {
		...
		<div class="pass_body2">
				<?php
					if (isset($success_msg)) {
						...
						echo '
							<form action="passRecover.php" method="POST">
								<p>Find your Buddies account!</p></br>
