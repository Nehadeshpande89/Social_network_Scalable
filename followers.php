...
<body>
	...
	echo '<li style="float: right;">
					...
					<div >
						...
						<li><a href="friends.php?u='.$username.'" style="background-color: #cdcdcd; color: darkblue">Friend</a></li>
	...
	if (($msg_count >=1 ) || ($username == $user)) {
			...
			$count_frnd_num = 0;
				...
				$queryfollowers = mysqli_query($con,$queryfollowers ) or die ("could not count");
				$countfollowers = mysqli_num_rows($queryfollowers );
				//count following
				$queryfollowing = "SELECT * FROM follow WHERE user_from='$username' ORDER BY id DESC";
				$queryfollowing= mysqli_query($con,$queryfollowing) or die ("could not count");
				...
				$queryfollowing2= mysqli_query($con,$queryfollowing2) or die ("could not count");
				while ($row=mysqli_fetch_array($queryfollowing2)) {
						$user_from = $row['user_from'];
						$user_to = $row['user_to'];
						//user to if friend 
						
						$if_user_to_follow = mysqli_query($con,"SELECT * FROM follow WHERE (user_from='$user_to' AND user_to='$user_from')");
				...
				echo '<div>
					     <table class="friendsmenu">
							<tr>
								...
								<th style="background-color: darkblue;"><a href="followers.php?u='.$username.'"><h1>Followers </br>'.$countfollowers.'</h1></a></th>
							</tr>
						</table>
					</div>';
					if ($countfollowers == 0){
					...
					while ($row=mysqli_fetch_array($queryfollowers )) {
						...
						$user_to = $row['user_to'];
						//spacif user profile
						$family_query = mysqli_query($con,"SELECT id,username,first_name,profile_pic,cover_pic,city,hometown,company,school,gender,verify_id FROM users where username='$user_from' ORDER BY id DESC LIMIT 1");
...
//check for userfrom propic delete
						$pro_changed = mysqli_query($con,"SELECT * FROM posts WHERE added_by='$user_from' AND (discription='updated his cover photo.' OR discription='updated her cover photo.') ORDER BY id DESC LIMIT 1");
						$get_pro_changed = mysqli_fetch_assoc($pro_changed);
		$pro_num = mysqli_num_rows($pro_changed);
		if ($pro_num == 0) {
						...
						echo '
							<div class="user_search_result_box" >';
								...
								if ($school == "") {
										...
										echo 'Work at <span style="color: darkblue;">'.$company.'</span><br>' ;
									}
								}else {
									echo 'Study at <span style="color: darkblue;">'.$school.'</span><br>' ;
								}if ($city == "") {
										if ($hometown == "") {
										//nothing
									}else {
										echo 'From <span style="color: darkblue;">'.$hometown.'</span>' ;
									}
								}else {
									echo 'Lives in <span style="color: darkblue;">'.$city.'</span><br>' ;
								}
								echo '
								</div>
							</div>
