...
if (isset($_POST['login'])) {
		if (isset($_POST['user_login']) && isset($_POST['password_login'])) {
			...
			$result = mysqli_query($con,"SELECT * FROM users WHERE (username='$user_login' || email='$user_login') AND password='$password_login_md5' AND activated='1' AND blocked_user='0'");
			...
			if ($num>0) {
				...
				$result1 = mysqli_query($con,"SELECT * FROM users WHERE (username='$user_login' || email='$user_login') AND password='$password_login_md5' AND activated ='0'");
				...
				if ($num1>0) {
					...
					$success_message = '
						...
						<form action="signin.php" method="POST">
							...
							<input class="submRecov" type="submit" name="submconfrmCode" id="senddata" value="Continue Buddies">
						</form>
						</div>
						';
						//header('location: signin.php');
				}else {
					$result1 = mysqli_query($con,"SELECT * FROM users WHERE (username='$user_login' || email='$user_login') AND password='$password_login_md5' AND blocked_user='1'");
					$num1 = mysqli_num_rows($result1);
					if ($num1>=1) {
						$success_message = '
...
if (isset($_POST['submconfrmCode'])) {
	$user_confrmCode_db = mysqli_real_escape_string($con,$_POST['confrmCode']);
	$user_loginnn = $_SESSION['user_loginn'];
	$result2 = mysqli_query($con,"SELECT * FROM users WHERE username='$user_loginnn' AND confirmCode='$user_confrmCode_db' AND activated='0'");
	$num2 = mysqli_num_rows($result2);
	$get_user_info_f = mysqli_fetch_assoc($result2);
	if ($num2>=1) {
		$password_update_query = mysqli_query($con,"UPDATE users SET activated='1', confirmCode='0' WHERE username='$user_loginnn'");
...
if(isset($_POST["name2check"]) && $_POST["name2check"] != ""){
    ...
    $sql_uname_check = mysqli_query($con,"SELECT id FROM users WHERE username='$username' LIMIT 1"); 
    ...
    if ($uname_check < 1) {
	    echo '<p style="color: darkblue; font-size: 13px; font-weight: 600; text-align: center; margin: 3px 0;">Success! Remember username for login</p>';
	    exit();
    } else {
...
$_POST['username'] = preg_replace('/\s+/','',$_POST['username']);
	try {
		...
		//username check
		$u_check = mysqli_query($con,"SELECT username FROM users WHERE username='$u_name'");
		$check = mysqli_num_rows($u_check);
		// Check if email already exists
		$e_check = mysqli_query($con,"SELECT email FROM users WHERE email='$u_email'");
			...
			if ($check == 0 ) {
				if ($email_check == 0) {
					if (strlen($_POST['password']) >4 ) {
						...
						";
						//if (@mail($_POST['email'],"Buddies Activation Code",$msg, "From:Buddies <no-reply@daowat.com>")) {
							
						$result = mysqli_query($con,"INSERT INTO users (first_name,username,email,password,gender,sign_up_date,activated) VALUES ('$_POST[first_name]','$_POST[username]','$_POST[email]','$_POST[password]','$_POST[gender]','$d','1')");
						$_SESSION['user_loginn'] = $_POST['username'];
						
						//sent follow
						//$user_from = $_POST['username'];
						//$user_to = 'nur';
						//$create_followMe = mysqli_query($con,"INSERT INTO follow VALUES ('', '$user_from', '$user_to', NOW(), 'no')");
						//$create_followFrom = mysqli_query($con,"INSERT INTO follow VALUES ('', '$user_to', '$user_from', NOW(), 'no')");
						//send message
						//$msg_body = 'Welcome !';
						//$msgdate = date("Y-m-d");
						//$opened = "no";
						//$messages = mysqli_query($con,"INSERT INTO pvt_messages VALUES ('','$user_to','$user_from','$msg_body','$msgdate','NOW()','$opened', '')");
...
$getposts = mysqli_query($con,"SELECT * FROM daowat WHERE photos != '' ORDER BY  RAND() ");
...
<html>
	<head>
	<title>Connecting Friends</title>
	<meta charset="uft-8">
	<link rel="icon" href="./img/title.png" type="image/x-icon">
	<link rel="image_src" href="http://www.daowat.com/userdata/daowat_pics/nur/1453542261.jpg" /><!--formatted-->
	<link rel="stylesheet" type="text/css" href="./css/style.css">
	<meta name="description" content="Welcome !. Join Buddies. Connect with your family. Buddies in overall world. Get update over the world." /><!--formatted-->
		...
		<div class="main">
			<div class="headerLogin">
				<div class="login_menubar clearfix">
					<div class="menu_logo">
						<h1>
							<a title="Go to Buddies Home" href="index.php">
								<b>Buddies</b>
							</a>
						</h1>
					</div>
							...
							<div class="maincontent">
							   <?php
								if (isset($success_message)) {
									...
									echo '
									   <h2><font face="bookman">Welcome !!</font></h2>
										<div class="maincontent_text">
										<font face="bookman">Join Buddies.com <br>
											<li>Connect with your family.</li>
											<li>Buddies in overall world.</li>
								...
								<div>
									<form action="" method="POST" class="registration">
										<div class="signup_form">
										    ...
										    <div class="g-recaptcha" style="margin-bottom : 20px" data-sitekey="6LeIxAcTAAAAAJcZVRqyHh71UMIEGNQ_MXjiZKhI"></div>
