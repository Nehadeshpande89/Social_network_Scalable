...
if (($_GET['u'] && $_GET['code']) == NULL) {
			...
			$check_unameMail = mysqli_query($con,"SELECT * FROM users WHERE username='$unameMail' AND confirmCode!='0' AND confirmCode='$uCodeMail'");
			...
			if ($check_unameMail_num >= 1) {
				$chengePass_cnfrm = "
							<form action='confirmationPass.php?u=".$unameMail."&code=".$uCodeMail."' method='POST'>
								<p>Change Your Buddies Password!</p></br>
				...
				if ($newpassword == $repear_password) {
					...
					if (mysqli_query($con,$updatePassQuery)) {
						...
						$msg = "Welcome !  ".$get_first_nm_fetch_query."
								successfully your Buddies password has been changed.
								";
						mail($get_email_fetch_query,"Buddies Password Changed",$msg, "From:Buddies <no-reply@daowat.com>");
					}else {
