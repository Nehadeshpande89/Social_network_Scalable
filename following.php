	...
	if (($msg_count >=1 ) || ($username == $user)) {
				...
				echo '<div>
					     <table class="friendsmenu">
							<tr>
								...
								<th style="background-color: darkblue;"><a href="following.php?u='.$username.'" ><h1>Following </br>'.$countfollowing.'</h1></a></th>
					...
					while ($row=mysqli_fetch_array($queryfollowing)) {
						...
						$user_to = $row['user_to'];
						//spacif user profile
						$family_query = mysqli_query($con,"SELECT id,username,first_name,profile_pic,cover_pic,city,hometown,company,school,gender,verify_id FROM users where username='$user_to' ORDER BY id DESC LIMIT 1");
		...
		if ($pro_changed_db != $profile_picuser_to_db) {
			...
			$profile_picuser_to = "userdata/profile_pics/".$profile_picuser_to_db;
						...
						$pro_changed = mysqli_query($con,"SELECT * FROM posts WHERE added_by='$user_to' AND (discription='updated his cover photo.' OR discription='updated her cover photo.') ORDER BY id DESC LIMIT 1");
						$get_pro_changed = mysqli_fetch_assoc($pro_changed);
		$pro_num = mysqli_num_rows($pro_changed);
		if ($pro_num == 0) {
