		...
		if ($postby_gender_value == 2) {
			if (($posted_to == $user) || ($user_gender_value == 2)) {
				...
				<form action='comment_frame.php?id=".$getid."' method='POST' name='postComment".$getid."'>
					<input style='padding: 10px 3px; min-width: 60px; width: 73%; margin: 0 0 5px 0; resize: none; border: 1px solid darkblue;' name='post_body' placeholder= 'Leave your comment here!' autofocus>
			...
			if ($female_msg >=1 ) {
				echo "
					<div style='margin: 0 52px;'>
					<form action='comment_frame.php?id=".$getid."' method='POST' name='postComment".$getid."'>
						<input style='padding: 10px 3px; min-width: 60px; width: 73%; margin: 0 0 5px 0; resize: none; border: 1px solid darkblue;' name='post_body' placeholder= ''>
						<input type='submit' name='postComment".$getid."' class='commentSubmit' value='Comment'>
					</form>
					</div>
				";
