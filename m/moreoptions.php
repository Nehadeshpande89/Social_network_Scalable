...
if (isset($_REQUEST['pid'])) {
	...
	$get_sharepost = mysqli_query($con,"SELECT * FROM posts WHERE id='$id'");
...
if (isset($_POST['continue']) and ! empty($_POST['continue'])) {
    if (isset($_POST['action_key'])) {
		...
		//delete all like
		$resultdl = mysqli_query($con,"DELETE FROM post_likes WHERE post_id='$id'");
		//delete share post comment like
		$getshareposts = mysqli_query($con,"SELECT * FROM posts WHERE share_post='$id'") or die(mysqli_error());
		while ($row_share = mysqli_fetch_assoc($getshareposts)) {
		$spostid= $row_share['id'];
		$resultdlt = mysqli_query($con,"DELETE FROM post_comments WHERE post_id='$spostid'");
		$resultdl = mysqli_query($con,"DELETE FROM post_likes WHERE post_id='$spostid'");
		}
		//delete all comments
		$resultd2 = mysqli_query($con,"DELETE FROM post_comments WHERE post_id='$id'");
		//delete share post
		$result1 = mysqli_query($con,"DELETE FROM posts WHERE share_post='$id'");
		//delete post
		$result3 = mysqli_query($con,"DELETE FROM posts WHERE id='$id'");
		...
		//reporting post
		$result = mysqli_query($con,"UPDATE posts SET report='1' WHERE id='$id'");
...
<div class="pro_body">
	...
	<div style="width: 100%;">
		<?php
			...
			$getpostsu_name = mysqli_fetch_assoc($getposts);
											...
											<tbody>
												<tr>
													...
													<td>
														<div class="v">
															<div>
																<span class="c_b">Add to Buddies</span>
															</div>
															<div>
																<span class="c_g">This post show in Buddies timeline.</span>
															</div>
														</div>
													</td>
												</tr>
