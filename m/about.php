	...
	$about_query = mysqli_query($con,"SELECT school,concentration,city,hometown,queote,bio,company,position,mobile,pub_email,relationship FROM users WHERE username='$username'");
		...
		echo '<div class="uiaccountstyle">
				<p class="uiaccountstyleHdr">WORK AND EDUCATION';
					if ($user==$username) {
						echo '<a href="editprofile.php?edit=work_education" style="float: right; text-decoration: none; font-size: 12px; color: darkblue">Edit</a>';	
					}else {
				...
				if ($user_company == '' && $user_position == '') {
					...
					echo '
				<p style="font-size: 15px; font-weight: bold; color: darkblue;">'.$user_company.'<br></p>
				<p style=" font-weight: bold; color: #7B7B7B;">'.$user_position.'<br></p>
				 ...
				 if ($school_name_user == '' && $concentration_name_user == '')  {
					...
					echo '
				<p style="font-size: 15px; font-weight: bold; color: darkblue;">'.$school_name_user.'<br></p>
					';
				}else if ($school_name_user == '' && $concentration_name_user != '')  {
					echo '
			...
			echo '
					...
					if ($user==$username) {
						echo '<a href="editprofile.php?edit=editcontact" style="float: right; text-decoration: none; font-size: 12px; color: darkblue">Edit</a>';	
					}else {
			...
			echo '</p>
				...
				if ($user_mobile == '' && $user_pub_email == '') {
					...
					echo '
						<p style="font-size: 15px; font-weight: bold; color: darkblue;">'.$user_pub_email.'</p>
						<p style=" font-weight: bold;  color: #7B7B7B;">Public Email</p>
					';
				}else if ($user_mobile != '' && $user_pub_email == '')  {
					echo '
						<p style="font-size: 15px;  font-weight: bold; color: darkblue;">'.$user_mobile.'</p>
			...
			<p class="uiaccountstyleHdr">CURRENT CITY AND HOMETOWN';
					if ($user==$username) {
						echo '<a href="editprofile.php?edit=editlocation" style="float: right; text-decoration: none; font-size: 12px; color: darkblue">Edit</a>';	
					}else {
			...
			echo '</p>
				...
				if ($city_name_user == '' && $hometown_name_user == '') {
					...
					echo '
						<p style="font-size: 15px; font-weight: bold; color: darkblue;">'.$hometown_name_user.'</p>
						<p style=" font-weight: bold; color: #7B7B7B;">Hometown</p>
					';
				}else if ($city_name_user != '' && $hometown_name_user == '')  {
					echo '
						<p style="font-size: 15px; font-weight: bold; color: darkblue;">'.$city_name_user.'</p>
				...
				echo '<p class="uiaccountstyleHdr">DETAILS ABOUT';
						if ($user==$username) {
							echo '<a href="editprofile.php?edit=editdetailsabout" style="float: right; text-decoration: none; font-size: 12px; color: darkblue">Edit</a>';	
						}else {
							
						}
				echo '</p>
					<div style="padding: 10px;">
					<p style=" color: darkblue; font-size: 14px; line-height: 18px; "> '.nl2br($user_bio).'<br></p>
				</div>';

			echo '<p class="uiaccountstyleHdr">FAVORITE QUEOTE';
					if ($user==$username) {
						echo '<a href="editprofile.php?edit=editqueote" style="float: right; text-decoration: none; font-size: 12px; color: darkblue">Edit</a>';	
					}else {
						
					}
			echo '</p>
				<div style="padding: 10px;">
				<p style=" color: darkblue; font-size: 14px; line-height: 18px; ">'.nl2br($user_queote).'<br></p>
			</div></div></div>';
		}else {
