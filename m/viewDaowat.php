...
<html>
<head>
	<title>View Buddies - Buddies</title>
	<meta charset="utf-8" />
...
//for getting post
$getposts = mysqli_query($con,"SELECT * FROM daowat WHERE id ='$id'") or die(mysqli_error());
...
while ($row = mysqli_fetch_assoc($getposts)) {
		...
		$get_comments = mysqli_query($con,"SELECT * FROM daowat_comments WHERE daowat_id='$id' ORDER BY id DESC");
		...
		//getting all like
		$get_like = mysqli_query($con,"SELECT * FROM dwt_likes WHERE dwt_id='$id' ORDER BY id DESC");
		...
		//showing data on profile
				echo "<div class='postBody post_search_result_box'>";
						...
						$like_query = mysqli_query($con,"SELECT * FROM dwt_likes WHERE user_name='$user' AND dwt_id='$id' ORDER BY id DESC LIMIT 1");
						$rows_uname = mysqli_num_rows($like_query);
						if ($rows_uname == 1) {
							echo "<a href='dlikesppl.php?dlikep=".$id."' style='color: darkblue;' >Liked ($count_like)</a>";
