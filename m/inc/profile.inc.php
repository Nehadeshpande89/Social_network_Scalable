...
<?php 
	...
	if (@($_POST['unfollow'])) {
		...
		if ( $following_found >= 1 ) {
			$delete_follow = mysqli_query($con,"DELETE FROM follow WHERE user_from='$user' && user_to='$username' ");
		}else{
			header("location: profile.php?u=$username");
		}
	}
	...
	if (@($_POST['unfriend'])) {
		...
		$following_found = mysqli_num_rows($if_following);
		if ( $following_found >= 1 ) {
			$delete_follow1 = mysqli_query($con,"DELETE FROM follow WHERE user_from='$user' && user_to='$username' ");
			$delete_follow2 = mysqli_query($con,"DELETE FROM follow WHERE user_from='$username' && user_to='$user' ");
		}else{
			header("location: profile.php?u=$username");
		}
	}
