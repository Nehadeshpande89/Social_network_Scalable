				...
				if (isset($_GET['u'])) {
					...
					if (ctype_alnum($username)) {
						...
						if (mysqli_num_rows($check)===1) {
							...
							$pro_changeds = mysqli_query($con,"SELECT * FROM posts WHERE added_by='$username' AND (discription='changed his profile picture.' OR discription='changed her profile picture.') ORDER BY id DESC LIMIT 2");
							$get_pro_changeds = mysqli_fetch_assoc($pro_changeds);
							$pro_nums = mysqli_num_rows($pro_changeds);
							if ($pro_nums != 0) {
							...
							if ($username != $user) {
								if (isset($_POST['submit'])) { 
									...
									$messages = mysqli_query($con,"INSERT INTO pvt_messages VALUES ('','$user','$username','$msg_body','$date','NOW()','$opened','')");
								...
								$getonlineuserTime = mysqli_query($con,"SELECT chatOnlineTime FROM users WHERE username='$username'") or die(mysqli_error());
								$rowTime = mysqli_fetch_assoc($getonlineuserTime);
								...
								*/
								...
								$get_message = mysqli_query($con,"SELECT * FROM pvt_messages WHERE (user_from='$user' AND user_to='$username') OR (user_from='$username' AND user_to='$user') ORDER BY id ASC LIMIT 50");
								...
								while ($msg = mysqli_fetch_assoc($get_message)) {
									...
									while ($row = mysqli_fetch_assoc($emoticonQuery)) {
										...
										<div style='display: flex; padding: 8px 0; border-top: 1px solid #ddd;''>
											<div style='width: 100%;'>
												<b><a href='profile.php?u=$user_by' style='text-decoration: none; font-size: 14px; color: darkblue;' title=\"Go to $posted_by's Profile\" class='posted_by'>$posted_by</a></b>
