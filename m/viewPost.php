...
if (isset($_REQUEST['pid'])) {
	...
	$id = mysqli_real_escape_string($con,$_REQUEST['did']);
}else {
...
while ($row = mysqli_fetch_assoc($getposts)) {
		...
		if($sharepostid != 0) {
			if($sp_added_by == $added_by) {
				if ($gender_user_db == '1') {
					$discription = "shared his <a href='viewPost.php?pid=".$sharepostid."' style='text-decoration: none; color: darkblue;'>post</a>";
				}else if ($gender_user_db == '2') {
					$discription = "shared her <a href='viewPost.php?pid=".$sharepostid."' style='text-decoration: none; color: darkblue;'>post</a>";
				}
			}else {
				$discription = "shared ".$sp_add_by." <a href='viewPost.php?pid=".$sharepostid."' style='text-decoration: none; color: darkblue;'>post</a>";
			}
		}
		...
		//showing data on profile
				echo "<div class='postBody post_search_result_box'>";
					...
					if ($user_posted_to == $added_by) {
						...
						if ($rows_uname == 1) {
							echo "<a href='like.php?upid=".$id."' style='color: darkblue;' >Liked</a>";
							echo "<a href='plikesppl.php?plikep=".$id."' style='color: darkblue;' >($count_like)</a>";
						}else {
