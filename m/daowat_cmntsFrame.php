...
if (isset($_POST['daowatComment' . $getid . ''])) {
	...
	$query = mysqli_query($con,"SELECT id,added_by  FROM daowat WHERE id='$getid'");
	$query_row = mysqli_fetch_assoc($query);
	$daowat_to = $query_row['added_by'];
	$insertPost = mysqli_query($con,"INSERT INTO daowat_comments VALUES ('','$daowat_body','$date_added',NOW(),'$user','$daowat_to','no','$getid')");
}
}
//post query
	$query = mysqli_query($con,"SELECT id,added_by  FROM daowat WHERE id='$getid'");
	...
	$dwtby_query = mysqli_query($con,"SELECT * FROM users WHERE username='$daowat_to'");
	...
	if ($dwtby_gender_value == 2) {
		if (($daowat_to == $user) || ($user_gender_value == 2)) {
			...
			<form action='daowat_cmntsFrame.php?id=".$getid."' method='POST' name='daowatComment".$getid."'>
				<input style='padding: 10px 3px; min-width: 60px; width: 73%; margin: 0 0 5px 0; border: 1px solid darkblue;' name='daowat_body' placeholder= 'Leave your comment here!' autofocus>
				<input type='submit' name='daowatComment".$getid."' class='commentSubmit' value='Comment'>
			</form>
			</div>
		";
	}else {
		$get_msg_num = mysqli_query($con,"SELECT * FROM pvt_messages WHERE user_from='$daowat_to' AND user_to='$user' LIMIT 2");
			$female_msg = mysqli_num_rows($get_msg_num);
			if ($female_msg >=1 ) {
				echo "
					<div>
					<form action='daowat_cmntsFrame.php?id=".$getid."' method='POST' name='daowatComment".$getid."'>
						<input style='padding: 10px 3px; min-width: 60px; width: 73%; margin: 0 0 5px 0; resize: none; border: 1px solid darkblue;' name='daowat_body' placeholder= ''>
						<input type='submit' name='daowatComment".$getid."' class='commentSubmit' value='Comment'>
					</form>
					</div>
				";
...
<?php
//Get relevant daowat
$get_comments = mysqli_query($con,"SELECT * FROM daowat_comments WHERE daowat_id='$getid' ORDER BY id DESC");
...
while ($comment = mysqli_fetch_assoc($get_comments)) {
	...
	$dwt_user_info = mysqli_query($con,"SELECT * FROM users WHERE username='$daowat_by'");
