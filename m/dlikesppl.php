...
<div class="pro_body">
	...
	<div style="width: 100%;">
		...
		<h2 style='font-size: 20px';>Buddies Like</h2><br />
			<?php 
			$user_from = "";
			//grab the message for the logged in user
			$grab_post = mysqli_query($con,"SELECT * FROM dwt_likes WHERE dwt_id='$getid' ORDER BY id DESC");
			$numrows = mysqli_num_rows($grab_post);
			if ($numrows > 0) {
			while ($get_like = mysqli_fetch_assoc($grab_post)) {
					...
					<div style='display: flex; padding: 8px 0; border-top: 1px solid #ddd;'> ";
						...
						echo "<div><b style='font-size: 14px; font-weight: bold; color: darkblue;'>$msg_by_fname</b> <br>";
							if ($user == $user_from) {
								...
								$check_if_follow = mysqli_query($con,"SELECT * FROM follow WHERE (user_from='$user' AND user_to='$user_from') ORDER BY id DESC LIMIT 2");
								$num_follow_found = mysqli_num_rows($check_if_follow);
								if ( $num_follow_found != "" ) {
									$check_if_friend = mysqli_query($con,"SELECT * FROM follow WHERE (user_from='$user_from' AND user_to='$user') ORDER BY id DESC LIMIT 2");
									...
									$if_uname_follow = mysqli_query($con,"SELECT * FROM follow WHERE (user_from='$user_from' AND user_to='$user') ORDER BY id DESC LIMIT 2");
