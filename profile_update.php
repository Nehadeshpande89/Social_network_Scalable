...
if (isset($_POST['uploadcov'])) {
	...
	if (((@$_FILES['coverpic']['type']=='image/jpeg') || (@$_FILES['coverpic']['type']=='image/png') || (@$_FILES['coverpic']['type']=='image/gif')) && (@$_FILES['coverpic']['size'] < 20000000)) {
		$chare = $user;
		...
		if (file_exists("userdata/profile_pics/$chare/".@$_FILES["coverpic"]["name"])) {
			...
			$cover_pic_query = mysqli_query($con,"UPDATE users SET cover_pic='$chare/$filename' WHERE username='$user' ");
...
if (isset($_POST['uploadpro'])) {
	...
	if (((@$_FILES['profilepic']['type']=='image/jpeg') || (@$_FILES['profilepic']['type']=='image/png') || (@$_FILES['profilepic']['type']=='image/gif')) && (@$_FILES['profilepic']['size'] < 20000000)) {
		$chare = $user;
		if (file_exists("userdata/profile_pics/$chare")) {
			//nothing
		}else {
		...
		if (file_exists("userdata/profile_pics/$chare/".$filename)) {
			...
			$sqlCommand = "INSERT INTO posts(date_added,added_by,user_posted_to,discription,photos,newsfeedshow) VALUES('$date_added','$added_by', '$user_posted_to','$discription', '$photos','1')";
			$query = mysqli_query($con,$sqlCommand) or die (mysqli_error());
			$profile_pic_query = mysqli_query($con,"UPDATE users SET profile_pic='$chare/$filename' WHERE username='$user' ");
			header("Location: profile.php?u=$user");
		}
	}
	else {
	...
	<ul>
		...
		<li style="float: left;">
			<div class="settingsleftcontent">
				<ul>
					<li><a href="profile_update.php" style="background-color: darkblue; border-radius: 3px; color: #fff;">Profile Update</a></li>
