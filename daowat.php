...
$num = mysqli_num_rows($result);
	if ($num == 1) {
		include ( "./inc/header.inc.php");
	    	...
	    	echo '<li style="float: right;">
					...
					<div >
						<nav>
						<ul>
						<li><a href="daowat.php?u='.$username.'" style="background-color: #cdcdcd; color: darkblue">Buddies</a></li>
		...
		$daowat =  trim($daowat);
		$daowat = mysqli_real_escape_string($con,$daowat);
		...
		if ($pic != "") {
			...
			if (((@$_FILES['uploadFile']['type']=='image/jpeg') || (@$_FILES['uploadFile']['type']=='image/png') || (@$_FILES['uploadFile']['type']=='image/gif')) && (@$_FILES['uploadFile']['size'] < 20000000)) {
				$chare = $user;
		...
		?>
			...
			<?php echo $error ?>
				<div style="width: 560px; margin: 15px auto;">
					<?php 
						if ($user == $username) {
							echo "
								...
								<input type='submit' name='uploadpic' value='Buddies' class='p_postSubmit' >
								</div>
								</form>
								</div>
							";
					...
					<?php
					//for getting post
					$getposts = mysqli_query($con,"SELECT * FROM posts WHERE added_by ='$username' AND (daowat_body !='' || daowat_post != '0') ORDER BY id DESC LIMIT 5 ") or die(mysqli_error());
