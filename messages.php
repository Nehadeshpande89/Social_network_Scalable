...
//chat count 
$buddyList = '0';
$getonlineuser = mysqli_query($con,"SELECT first_name,username FROM users WHERE (chatOnlineTime>=now()-300)") or die(mysqli_error());

if (mysqli_num_rows($getonlineuser) >= '2' ) {
	while ($row = mysqli_fetch_assoc($getonlineuser)) {
		$usrnm= $row['username'];
		$check_if_friend = mysqli_query($con,"SELECT * FROM follow WHERE (user_from='$usrnm' AND user_to='$user') || (user_from='$user' AND user_to='$usrnm') ORDER BY id DESC LIMIT 2");
	...
	<ul>
		<li style="float: left;">
			...
			<div>
				...
				$grab_messages = mysqli_query($con,"SELECT * FROM pvt_messages WHERE user_to='$user' OR user_from='$user' ORDER BY id DESC");
				...
				while ($get_msg = mysqli_fetch_assoc($grab_messages)) {
					...
					$get_user_info = mysqli_query($con,"SELECT * FROM users WHERE username='$user_from'");
					...
					$get_userto_info = mysqli_query($con,"SELECT * FROM users WHERE username='$user_to'");
					...
					//check for userfrom propic delete
						$pro_changed = mysqli_query($con,"SELECT * FROM posts WHERE added_by='$user_from' AND (discription='changed his profile picture.' OR discription='changed her profile picture.') ORDER BY id DESC LIMIT 1");
		...
		if ($pro_changed_db != $profile_picuser_from_db) {
			...
			$profile_picuser_from = "userdata/profile_pics/".$profile_picuser_from_db;
					...
					//check for userto propic delete
						$pro_changed = mysqli_query($con,"SELECT * FROM posts WHERE added_by='$user_to' AND (discription='changed his profile picture.' OR discription='changed her profile picture.') ORDER BY id DESC LIMIT 1");
						$get_pro_changed = mysqli_fetch_assoc($pro_changed);
		$pro_num = mysqli_num_rows($pro_changed);
		if ($pro_num == 0) {
			...
			$profile_picuser_to = "userdata/profile_pics/".$profile_picuser_to_db;
					...
					if (strlen($msg_body) > 50) {
						...
						if ($user_from == $user) {
							...
							echo "<div style='margin-left: 65px;'><b><a href='profile.php?u=$user_to' style='text-decoration: none; font-size: 14px; font-weight: bold; color: darkblue;'>$msg_to_fname</a></b> <br>
							...
							echo "<div style='margin-left: 65px;'><b><a href='profile.php?u=$user_from' style='text-decoration: none; font-size: 14px; font-weight: bold; color: darkblue;'>$msg_by_fname</a></b> <br>
		...
		<li style="float: right;">
			<div>
				...
				if (isset($_GET['u'])) {
					...
					if (ctype_alnum($username)) {
						//check user exists
						$check = mysqli_query($con,"SELECT username, id,first_name FROM users WHERE username='$username'");
						if (mysqli_num_rows($check)===1) {
							$get = mysqli_fetch_assoc($check);
							$username = $get['username'];
							...
							if ($username != $user) {
								if (isset($_POST['submit'])) { 
									...
									$msg_body = mysqli_real_escape_string($con,$msg_body);
									$date = date("Y-m-d");
									$opened = "no";
									...
									$messages = mysqli_query($con,"INSERT INTO pvt_messages VALUES ('','$user','$username','$msg_body','$date','','$opened','')");
								...
								$get_user_gender = mysqli_query($con,"SELECT * FROM users WHERE username='$user'");
								$gender_user_row = mysqli_fetch_assoc($get_user_gender);
								$user_gender_value = $gender_user_row['gender'];
								//gettting username gender
								$get_gender = mysqli_query($con,"SELECT * FROM users WHERE username='$username'");
								...
								$get_message = mysqli_query($con,"SELECT * FROM pvt_messages WHERE (user_from='$user' AND user_to='$username') OR (user_from='$username' AND user_to='$user') ORDER BY id DESC LIMIT 50");
								$count = mysqli_num_rows($get_message);
								//deleting msg
								if ($count >= 51) {
									$result = mysqli_query($con,"DELETE FROM pvt_messages WHERE (user_from='$user' AND user_to='$username') OR (user_from='$username' AND user_to='$user') ORDER BY id ASC LIMIT 1");
								}
								if ($count != 0) {
								while ($msg = mysqli_fetch_assoc($get_message)) {
									...
									$get_user_info = mysqli_query($con,"SELECT * FROM users WHERE username='$user_by'");
									$get_info = mysqli_fetch_assoc($get_user_info);
									$profile_pic_db= $get_info['profile_pic'];
									$posted_by = $get_info['first_name'];
									//making emo
									$emoticonQuery = mysqli_query($con,"SELECT * FROM emoticons");
						...
						$pro_changed = mysqli_query($con,"SELECT * FROM posts WHERE added_by='$user_by ' AND (discription='changed his profile picture.' OR discription='changed her profile picture.') ORDER BY id DESC LIMIT 1");
						$get_pro_changed = mysqli_fetch_assoc($pro_changed);
		$pro_num = mysqli_num_rows($pro_changed);
		if ($pro_num == 0) {
			...
			$profile_pic = "userdata/profile_pics/".$profile_pic_db;
										...
										<div style='margin: 10px;'>
											...
											<div style='margin: 0 0 0 50px;'>
												<b><a href='profile.php?u=$user_by' style='text-decoration: none;  padding-left: 0px; font-size: 14px; color: darkblue; font-weight: 700;' title=\"Go to $posted_by's Profile\" >$posted_by</a></b>
