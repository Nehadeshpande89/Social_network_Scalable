	...
	if ($num == 1) {
			...
			echo '<li style="float: right;">
					...
					<div >
						...
						<li><a href="note.php?u='.$username.'" style="background-color: #cdcdcd; color: darkblue">Note</a></li>
		...
		echo ''.$error.'
			<div style="width: 560px; margin: 0 auto;">';
				...
				//for getting note
					$noteshowmorelastid = "";
					if ($user == $username) {
						$getposts = mysqli_query($con,"SELECT * FROM posts WHERE user_posted_to ='$username' && note='1' ORDER BY id DESC LIMIT 5") or die(mysqli_error());
					}else {
						$getposts = mysqli_query($con,"SELECT * FROM posts WHERE user_posted_to ='$username' && note='1' && note_privacy='public' ORDER BY id DESC LIMIT 5") or die(mysqli_error());
					}
