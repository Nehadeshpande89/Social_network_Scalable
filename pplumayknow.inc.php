...
if($user_gender_value == 1) {
	$get_ppl_info = mysqli_query($con,"SELECT * FROM users WHERE username!='$user' AND gender='1' AND activated='1' AND blocked_user='0' ORDER BY  RAND() ");
	//declear variable
	$getuserNum= '0';
	...
	while ($row_user = mysqli_fetch_assoc($get_ppl_info)) {
			...
			//if follow	
			$if_user_to_follow = mysqli_query($con,"SELECT * FROM follow WHERE (user_from='$user' AND user_to='$user_name')");
			...
			if ($count_user_to_follow == 0) {
				...
				$pro_changed = mysqli_query($con,"SELECT * FROM posts WHERE added_by='$user_name' AND (discription='changed his profile picture.' OR discription='changed her profile picture.') ORDER BY id DESC LIMIT 1");
				...
				echo "
					...
					<div style='display: flex; padding: 8px 0;'> ";
						...
						echo "<div style='margin-left: 10px;'><b><a href='profile.php?u=$user_name' style='text-decoration: none; font-size: 14px; color: darkblue;' title=\"Go to $user_name_f's Profile\" class='posted_by'>$user_name_f</a></b> <br><br>
	...
	$get_user = mysqli_query($con,"SELECT * FROM users WHERE gender='2'");
